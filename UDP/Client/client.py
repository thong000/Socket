...
def receiveChunk(pipe, chunkSize, part, processPipe, server):
    ...
    while length < chunkSize:
        data += socketRecvDataWithSeq(client, server, 1024, 2, ack)
        length += 1024
...
try:
        ...
        size = socketRecvDataWithSeq(client, server_address, 1024, 1, ack)
        #print(size)
        data = socketRecvDataWithSeq(client, server_address, size, 0, ack)
            ...
            if isChange("Client/input.txt", oldSize):
                ...
                for i in range(len(split)):
                    if split[i] != "":
                        ...
                        filesize = socketRecvDataWithSeq(client, server_address, 1024, 1, ack)
                        ...
                        receiver1 = Process(target=receiveChunk,
                                                               args=(input_1, chunkSize, 1, p1))
                        receiver2 = Process(target=receiveChunk,
                                                                args=(input_2, chunkSize, 2, p2))
                        receiver3 = Process(target=receiveChunk,
                                                                args=(input_3, chunkSize, 3, p3))
                        receiver4 = Process(target=receiveChunk,
                                                                args=(input_4, filesize - 3 * chunkSize, 4, p4))
                        processes = [receiver1, receiver2, receiver3, receiver4]

                        for p in processes:
                            p.start()

                            printProcess(receiver1, receiver2, receiver3, receiver4, progress1, progress2, progress3, progress4, split[i])
                        ...
                        with open("Client/" + split[i],"wb") as f:
                                for data in output:
                                    f.write(data)
                                print(f"[INFO] Da ghi file {split[i]}")
