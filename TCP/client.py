...
def start_client(serverIP, serverPort, folder):
    ...
    try:
        ...
        while True:
            ...
            sizeChanged,oldSize = countChangedCharacters(folder + "input.txt", oldSize)

            if sizeChanged > 0:

                fileData = getFileData(folder + "input.txt")
                fileNames = splitString(fileData, '\n')
                ...
                for i in range(len(fileNames)):
                    ...
                    if not (fileNames[i] in downloaded):
                        ...
                        if fileSize != -1:  # Truong hop file khong co tren server
                            ...
                            output = [output_1.recv(), output_2.recv(), output_3.recv(),
                                      output_4.recv()]  # Data nhan duoc
